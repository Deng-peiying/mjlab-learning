...
class ViserPlayViewer(BaseViewer):
    ...
    def setup(self) -> None:
        ...
        with tabs.add_tab("Controls", icon=viser.Icon.SETTINGS):
            ...
            self._camera_overlays = ViserCameraOverlays(
                self._server, self.env, sim.mj_model
            )
                ...
                def _(_) -> None:
                    if self._ckpt_user_event.is_set():
                        self._actions.append(
                            (ViewerAction.FETCH_CHECKPOINT, "selected")
                        )
    ...
    def sync_env_to_viewer(self) -> None:
        ...
        has_pending_updates = (
            bool(self._pending_update_reasons) or self._scene.needs_update
        )
    ...
    def _update_status_display(self) -> None:
        ...
        capped = (
            ' <span style="color:#e74c3c;">[CAPPED]</span>' if status.capped else ""
        )
        error_line = ""
        if status.last_error:
            # Show last line of traceback to avoid flooding the panel.
            first_line = status.last_error.strip().splitlines()[-1]
            error_line = f'<br/><span style="color:#e74c3c;"><strong>Error:</strong> {first_line}</span>'
