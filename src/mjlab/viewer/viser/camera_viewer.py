...
class ViserCameraViewer:
    ...
    def __init__(
        ...
        if self._has_rgb:
            self._rgb_handle = self._server.gui.add_image(
                image=np.zeros(
                    (self._display_height, self._display_width, 3), dtype=np.uint8
                ),
    ...
    def _update_frustum(self, sim_data, env_idx: int, scene_offset: np.ndarray) -> None:
        ...
        cam_mat = (
            sim_data.cam_xmat[env_idx, cam_id].cpu().numpy().reshape(3, 3)
        )  # [3, 3]
    ...
    def update(
        ...
        if (
            self._has_depth
            and self._depth_handle is not None
            and data.depth is not None
        ):
        ...
        if (
            self._has_seg
            and self._seg_handle is not None
            and data.segmentation is not None
        ):
