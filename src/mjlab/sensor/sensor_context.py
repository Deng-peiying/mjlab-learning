...
class SensorContext:
    ...
    def _create_context(self, mj_model: mujoco.MjModel) -> None:
        ...
        has_any_rgb = self._rgb_adr_np is not None and any(
            a >= 0 for a in self._rgb_adr_np
        )
        has_any_depth = self._depth_adr_np is not None and any(
            a >= 0 for a in self._depth_adr_np
        )
        has_any_seg = self._seg_adr_np is not None and any(
            a >= 0 for a in self._seg_adr_np
        )
