    ...
    def edit_spec(self, scene_spec: mujoco.MjSpec, entities: dict[str, Entity]) -> None:
        ...
        for sensor in scene_spec.sensors:
            if sensor.name == self.cfg.prefixed_name:
                is_entity_scoped = (
                    self.cfg.obj is not None and self.cfg.obj.entity is not None
                )
                if is_entity_scoped:
                    raise ValueError(
        ...
        scene_spec.add_sensor(
            ...
            objtype=(
                _OBJECT_TYPE_MAP[self.cfg.obj.type]
                if self.cfg.obj is not None
                else None
            ),
            objname=(
                self.cfg.obj.prefixed_name() if self.cfg.obj is not None else None
            ),
            reftype=(
                _OBJECT_TYPE_MAP[self.cfg.ref.type]
                if self.cfg.ref is not None
                else None
            ),
            refname=(
                self.cfg.ref.prefixed_name() if self.cfg.ref is not None else None
            ),
