...
每种地形类型都有一个带有合理默认值的预设函数。可在调用处覆盖任意参数。
命名配置（ROUGH、STAIRS、ALL）由预设组合而成。

要添加新地形，请定义一个用 @terrain_preset 装饰的函数::
...
它将被自动包含在 ALL_TERRAIN_PRESETS 和 ALL_TERRAINS_CFG 中。
"""
...
def terrain_preset(fn: _F) -> _F:
    """将地形预设注册到 ALL_TERRAIN_PRESETS 中。"""
