  ...
  horizontal_scale if non-zero."""
    ...
    def function(
        ...
        return TerrainOutput(
            origin=origin, geometries=[geom], flat_patches=flat_patches
        )
    ...
    def function(
        ...
        if border_pixels > 0:
            ...
            hf_raw = amplitude_pixels * (
                np.cos(yy * wave_number) + np.sin(xx * wave_number)
            )
...
class HfPerlinNoiseTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        if border_pixels > 0:
            ...
            normalized_elevation = np.zeros(
                (width_pixels, length_pixels), dtype=np.float32
            )
            normalized_elevation[
            ...
            noise_min, noise_max = noise_raw.min(), noise_raw.max()
            noise_range = noise_max - noise_min if noise_max > noise_min else 1.0
            normalized_elevation = ((noise_raw - noise_min) / noise_range).astype(
                np.float32
            )
