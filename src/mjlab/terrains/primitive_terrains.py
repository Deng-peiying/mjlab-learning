...
class BoxRandomGridTerrainCfg(SubTerrainCfg):
  ...
  to reduce geom count."""
    ...
    def _create_merged_boxes(
        ...
        quantized_heights = (
            np.round(height_map / self.height_merge_threshold)
            * self.height_merge_threshold
        )
        ...
        for i in range(num_boxes_x):
            for j in range(num_boxes_y):
                ...
                while max_x < min(i + self.max_merge_distance, num_boxes_x):
                    if (
                        not visited[max_x, j]
                        and abs(quantized_heights[max_x, j] - height) < 1e-6
                    ):
                        max_x += 1
                    else:
                        break
                ...
                can_expand_y = True
                while (
                    max_y < min(j + self.max_merge_distance, num_boxes_y)
                    and can_expand_y
                ):
                    for x in range(i, max_x):
                        if (
                            visited[x, max_y]
                            or abs(quantized_heights[x, max_y] - height) > 1e-6
                        ):
                            can_expand_y = False
                            break
                    if can_expand_y:
                ...
                box_center_x = (
                    half_border_width + (i + (max_x - i) / 2) * self.grid_width
                )
                box_center_y = (
                    half_border_width + (j + (max_y - j) / 2) * self.grid_width
                )
...
class BoxRandomSpreadTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        if self.add_floor:
            ...
            geometries.append(
                TerrainGeometry(geom=floor_geom, color=(0.4, 0.4, 0.4, 1.0))
            )
        ...
        geometries.append(
            TerrainGeometry(geom=platform_geom, color=(0.4, 0.4, 0.4, 1.0))
        )
        ...
        for _ in range(num_boxes):
            ...
            pos_x = rng.uniform(
                self.border_width + size_x / 2,
                self.size[0] - self.border_width - size_x / 2,
            )
            pos_y = rng.uniform(
                self.border_width + size_y / 2,
                self.size[1] - self.border_width - size_y / 2,
            )
...
class BoxOpenStairsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        if self.inverted:
            ...
            platform_h_center = (
                num_steps + 1
            ) * step_height - 0.5 * self.step_thickness
    ...
    def function(
        ...
        for i in range(num_x):
            for j in range(num_y):
                ...
                # Randomized size.
                size_x = base_size + rng.uniform(
                    -stone_size_variation, stone_size_variation
                )
                size_y = base_size + rng.uniform(
                    -stone_size_variation, stone_size_variation
                )
...
class BoxNarrowBeamsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        for angle in angles:
            ...
            geometries.append(
                TerrainGeometry(geom=geom, color=brand_ramp(_MUJOCO_BLUE, 0.5))
            )
    ...
    def function(
        ...
        for i in range(num_boxes_x):
            ...
            for j in range(num_boxes_y):
                ...
                x_min, x_max = (
                    bx_center - self.grid_width / 2,
                    bx_center + self.grid_width / 2,
                )
                y_min, y_max = (
                    by_center - self.grid_width / 2,
                    by_center + self.grid_width / 2,
                )
...
class BoxNestedRingsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        geometries.append(
            TerrainGeometry(geom=box, color=brand_ramp(_MUJOCO_BLUE, 0.5))
        )
