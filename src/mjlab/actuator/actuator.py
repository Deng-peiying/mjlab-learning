...
class ActuatorCfg(ABC):
  ...
  on the same step."""
    ...
    def __post_init__(self) -> None:
        ...
        assert self.delay_update_period >= 0, (
            "delay_update_period must be non-negative."
        )
    ...
    def apply_delay(self, cmd: ActuatorCmd) -> ActuatorCmd:
        ...
        if cf == "position":
            ...
            return dataclasses.replace(
                cmd, position_target=self._delay_buffer.compute()
            )
        elif cf == "velocity":
            self._delay_buffer.append(cmd.velocity_target)
            return dataclasses.replace(
                cmd, velocity_target=self._delay_buffer.compute()
            )
        else:
