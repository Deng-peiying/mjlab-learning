        ...
        for path, body in _iter_body_paths(root):
            ...
            for g in body.geoms:
                if int(g.type) != mesh_type:
                    continue
                role_buckets[_classify_geom_role(g)].append(
                    _extract_variant_geom_spec(g)
                )
...
def _populate_dependent_fields(
    ...
    for entity_prefix, metadata in variant_info:
        ...
        for slot in metadata.slots:
            full_geom_name = _qualified_slot_geom_name(
                entity_prefix, slot.template_geom_name
            )
            gid = mujoco.mj_name2id(
                padded_model, mujoco.mjtObj.mjOBJ_GEOM, full_geom_name
            )
            if gid < 0:
                raise ValueError(
        ...
        for v_idx in range(len(metadata.variant_names)):
            ...
            for body_path, scene_bid in scene_body_id_by_path.items():
                ...
                body_subtreemass[worlds, scene_bid] = v_model.body_subtreemass[
                    source_bid
                ]
            ...
            for body_path, body in _iter_body_paths(source_root):
                ...
                for g in body.geoms:
                    ...
                    geom_aabb[worlds, scene_gid] = v_model.geom_aabb[
                        source_gid
                    ].reshape(2, 3)
        ...
        for v_idx, slot_specs in enumerate(metadata.variant_slot_specs):
            ...
            for s_idx, gspec in enumerate(slot_specs):
                ...
                if gspec.material:
                    ...
                    matid = mujoco.mj_name2id(
                        model, mujoco.mjtObj.mjOBJ_MATERIAL, full_mat_name
                    )
                    if matid < 0:
                        raise ValueError(
