...
class UniformVelocityCommandCfg(CommandTermCfg):
    ...
    """使用世界坐标系速度指令的环境比例。
  世界坐标系环境在世界坐标系中采样线速度，并在每步旋转到机体坐标系，
  使得指令方向在世界坐标系中保持固定。"""
    rel_forward_envs: float = 0.0
    """接收仅前进指令的环境比例（正 lin_vel_x，零 lin_vel_y 和 ang_vel_z）。
  增加直线行走的训练覆盖，这对爬楼梯很重要。"""
