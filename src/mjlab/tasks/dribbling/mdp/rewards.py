...
from mjlab.tasks.dribbling.mdp.terrain_utils import terrain_normal_from_sensors
...
def _planar_command_norm(command: torch.Tensor) -> torch.Tensor:
    """Return planar command magnitude, plus yaw rate when present."""
    norm = torch.norm(command[:, :2], dim=1)
    if command.shape[1] > 2:
        norm = norm + torch.abs(command[:, 2])
    return norm
...
def feet_air_time(
    ...
    if command_name is not None:
        ...
        if command is not None:
            total_command = _planar_command_norm(command)
