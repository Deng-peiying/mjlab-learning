        ...
        if "interval" in self._mode_term_cfgs:
            for index, term_cfg in enumerate(self._mode_class_term_cfgs["interval"]):
                if not term_cfg.is_global_time:
                    ...
                    sampled_interval = (
                        torch.rand(num_envs, device=self.device) * (upper - lower)
                        + lower
                    )
            ...
            if mode == "interval":
                ...
                if term_cfg.is_global_time:
                    ...
                    if len(valid_env_ids) > 0:
                        ...
                        lower, upper = term_cfg.interval_range_s
                        sampled_time = (
                            torch.rand(len(valid_env_ids), device=self.device)
                            * (upper - lower)
                            + lower
                        )
                        self._interval_term_time_left[index][valid_env_ids] = (
                            sampled_time
                        )
                ...
                if min_step_count == 0:
                    ...
                    last_triggered_step = self._reset_term_last_triggered_step_id[
                        index
                    ][env_ids]
                    triggered_at_least_once = self._reset_term_last_triggered_once[
                        index
                    ][env_ids]
                    ...
                    ) & ~triggered_at_least_once
                    if isinstance(env_ids, torch.Tensor):
                        valid_env_ids = env_ids[valid_trigger]
                    else:
                        valid_env_ids = valid_trigger.nonzero().flatten()
                    if len(valid_env_ids) > 0:
                        self._reset_term_last_triggered_once[index][valid_env_ids] = (
                            True
                        )
                        self._reset_term_last_triggered_step_id[index][
                            valid_env_ids
                        ] = global_env_step_count
                        term_cfg.func(self._env, valid_env_ids, **term_cfg.params)
                        fired = True
        ...
        for term_name, term_cfg in self.cfg.items():
            ...
            if term_cfg.mode == "interval":
                ...
                if term_cfg.is_global_time:
                    ...
                    lower, upper = term_cfg.interval_range_s
                    time_left = (
                        torch.rand(self.num_envs, device=self.device) * (upper - lower)
                        + lower
                    )
                    self._interval_term_time_left.append(time_left)
