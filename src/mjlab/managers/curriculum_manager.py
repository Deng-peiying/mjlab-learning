...
# 数据容器，省掉了手写 __init__ 的麻烦；kw_only=True 创建对象时，必须写字段名，不能靠位置猜
@dataclass(kw_only=True)
class CurriculumTermCfg(ManagerTermBaseCfg):
    """课程项的配置。

    课程项在训练过程中修改环境参数，以实现课程学习策略
    （例如，逐步增加任务难度）。
    """
...
class CurriculumManager(ManagerBase):
    ...
    课程管理器在训练过程中根据智能体表现更新环境参数。
    每个项可以修改任务难度的不同方面（例如，地形复杂度、指令范围）。
    """
