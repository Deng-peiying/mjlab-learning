...
def test_nan_guard_captures_and_dumps_on_nan(simple_model):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sim = Simulation(
            num_envs=4, cfg=cfg, model=simple_model, device=get_test_device()
        )
...
def test_nan_guard_detects_correct_env_ids(simple_model):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sim = Simulation(
            num_envs=10, cfg=cfg, model=simple_model, device=get_test_device()
        )
...
def test_nan_guard_saves_model(simple_model):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sim = Simulation(
            num_envs=2, cfg=cfg, model=simple_model, device=get_test_device()
        )
...
def test_nan_guard_captures_mocap_state(mocap_model):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sim = Simulation(
            num_envs=2, cfg=cfg, model=mocap_model, device=get_test_device()
        )

        # Set a known mocap position before stepping.
        sim.data.mocap_pos[0, 0] = torch.tensor(
            [1.0, 2.0, 3.0], device=get_test_device()
        )
