...
def test_effort_limits_accepts_operation_object(device):
    ...
    assert torch.allclose(
        env_str.sim.model.actuator_forcerange[0],
        env_obj.sim.model.actuator_forcerange[0],
    )
...
def test_pd_gains_rejects_unsupported_operation(device):
    ...
    with pytest.raises(ValueError, match="only supports 'scale' and 'abs'"):
        dr.pd_gains(
            env, ids, kp_range=(1.0, 1.0), kd_range=(1.0, 1.0), operation=dr.add
        )
...
def test_dof_armature_recompute_matches_recompile(device):
    ...
    sim = Simulation(
        num_envs=1, cfg=SimulationCfg(), model=original_model, device=device
    )
    sim.expand_model_fields(expand_fields)
    ...
    ref_dof_invweight = torch.tensor(
        recompiled_model.dof_invweight0, dtype=torch.float32
    )
    torch.testing.assert_close(
        gpu_dof_invweight, ref_dof_invweight, atol=1e-4, rtol=1e-4
    )
