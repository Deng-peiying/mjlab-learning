...
def test_basic_raycast_hit_detection(robot_with_floor_xml, device):
    ...
    assert torch.allclose(
        data.distances, torch.full_like(data.distances, 2.0), atol=0.1
    )
...
def test_raycast_normals_point_up(robot_with_floor_xml, device):
    ...
    scene, sim = make_scene_and_sim(
        device, robot_with_floor_xml, sensors=(raycast_cfg,)
    )
...
def test_raycast_exclude_parent_body(robot_with_floor_xml, device):
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=GridPatternCfg(
            size=(0.1, 0.1), resolution=0.1, direction=(0.0, 0.0, -1.0)
        ),
        max_distance=10.0,
        exclude_parent_body=True,
    )
...
def test_raycast_include_geom_groups(device):
  ...
  """
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=GridPatternCfg(
            size=(0.0, 0.0), resolution=0.1, direction=(0.0, 0.0, -1.0)
        ),
        max_distance=10.0,
    ...
    assert torch.allclose(
        data.distances, torch.full_like(data.distances, 3.0), atol=0.1
    )
...
def test_raycast_frame_attachment_geom(device):
  ...
  """
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        pattern=GridPatternCfg(
            size=(0.2, 0.2), resolution=0.1, direction=(0.0, 0.0, -1.0)
        ),
        max_distance=10.0,
    )
    ...
    assert torch.allclose(
        data.distances, torch.full_like(data.distances, 1.95), atol=0.1
    )
...
def test_raycast_frame_attachment_site(robot_with_floor_xml, device):
    ...
    assert torch.allclose(
        data.distances, torch.full_like(data.distances, 1.9), atol=0.1
    )
...
def test_raycast_different_direction(device):
  ...
  """
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=GridPatternCfg(
            size=(0.2, 0.2), resolution=0.1, direction=(1.0, 0.0, 0.0)
        ),
        max_distance=10.0,
    )
...
def test_raycast_hit_pos_w_correctness(robot_with_floor_xml, device):
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=GridPatternCfg(
            size=(0.4, 0.4), resolution=0.2, direction=(0.0, 0.0, -1.0)
        ),
        max_distance=10.0,
    )
...
def test_raycast_body_rotation_affects_rays(device):
  ...
  """
    ...
    assert torch.allclose(
        data_unrotated.distances,
        torch.full_like(data_unrotated.distances, 2.0),
        atol=0.1,
    )
...
def test_ray_alignment_yaw_singularity(device):
  ...
  """
    ...
    raycast_cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=GridPatternCfg(
            size=(0.0, 0.0), resolution=0.1, direction=(1.0, 0.0, -1.0)
        ),
        ray_alignment="yaw",
        max_distance=10.0,
    )
    ...
    sim_cfg = SimulationCfg(mujoco=MujocoCfg(gravity=(0, 0, 0)), njmax=20)
    scene, sim = make_scene_and_sim(
        device, xml, sensors=(raycast_cfg,), sim_cfg=sim_cfg
    )
...
def test_ring_pattern_num_rays(device):
    ...
    cfg = RayCastSensorCfg(
        ...
        pattern=RingPatternCfg.single_ring(
            radius=0.1, num_samples=8, include_center=True
        ),
    )
...
def test_ring_pattern_no_center(device):
    ...
    cfg = RayCastSensorCfg(
        ...
        frame=ObjRef(type="body", name="base", entity="robot"),
        pattern=RingPatternCfg.single_ring(
            radius=0.1, num_samples=4, include_center=False
        ),
    )
    simple_xml = """
    <mujoco>
