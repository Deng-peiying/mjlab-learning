...
def test_quat_zero_range_unchanged(quat_env, dr_func, field, asset_cfg, ids_attr):
    ...
    assert torch.allclose(
        result, default_quat.unsqueeze(0).expand_as(result), atol=1e-6
    )
...
def test_body_quat_only_specified_axes(quat_env):
    ...
    if torch.allclose(
        q_default, torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=env.device)
    ):
        assert torch.allclose(
            result[..., 1], torch.zeros_like(result[..., 1]), atol=1e-6
        )
        assert torch.allclose(
            result[..., 2], torch.zeros_like(result[..., 2]), atol=1e-6
        )
...
def test_pseudo_inertia_t1_shifts_com(inertia_env):
    ...
    assert torch.allclose(
        ipos[..., 0],
        (def_ipos[..., 0] + t).unsqueeze(0).expand_as(ipos[..., 0]),
        atol=1e-5,
    )
    assert torch.allclose(
        ipos[..., 1:],
        def_ipos[..., 1:].unsqueeze(0).expand_as(ipos[..., 1:]),
        atol=1e-5,
    )
