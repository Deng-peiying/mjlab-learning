    ...
    for slot in metadata.slots:
        ...
        if slot.key.role == "visual":
            ...
            assert conaffinity == 0, (
                f"visual slot {slot.key} has conaffinity={conaffinity}"
            )
        else:
...
def test_select_default_values_uses_per_world_variant_defaults():
    ...
    object_cfg = VariantEntityCfg(
        variants={
            ...
            "cone": lambda: _explicit_variant(
                "cone", 0.7, (4e-4, 5e-4, 6e-4), cone=True
            ),
        },
...
def test_viser_builds_per_world_mesh_handles_for_variants():
    ...
    try:
        ...
        mocap_pos = (
            env.sim.data.mocap_pos.cpu().numpy()
            if env.sim.mj_model.nmocap > 0
            else None
        )
        mocap_quat = (
            env.sim.data.mocap_quat.cpu().numpy()
            if env.sim.mj_model.nmocap > 0
            else None
        )
